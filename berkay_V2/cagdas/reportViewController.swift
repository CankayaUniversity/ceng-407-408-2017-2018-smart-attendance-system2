...
class reportViewController: UIViewController, UITableViewDelegate, UITableViewDataSource {
    ...
    var count = 0
    var contentCell = ["29.04.2018 No", "3.5.2018 No", "5.5.2018 No","7.5.2018 No","25.5.2018 No"]
    ...
    override func viewDidLoad() {
        ...
        docRef = db.collection("CheckIn").document("408").collection("25.5.2018").document(userMail)
        docRef.getDocument { (docSnapshot, Error) in
            guard let docSnapshot = docSnapshot, docSnapshot.exists else {print("RETURNED 5"); return}
            let myData = docSnapshot.data()
            // var userCheckIn = String()
            // userCheckIn = myData!["Attendence"] as! String
            self.count += 1
            print("ATTENDENCE TRUE")
            self.contentCell[4] = "25.5.2018 Yes"
            self.refresh()
        }
